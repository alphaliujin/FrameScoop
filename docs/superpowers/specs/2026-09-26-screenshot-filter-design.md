...
**前缀表里放的是「工具名的完整前缀」，不是「截图」这个词。**
这是个容易搞错的分层：

> macOS 系统截屏名是 `截屏<时间>.png`，工具名在前，所以前缀 `截屏` 成立。
> 但中文 IM 工具是 `微信截图<时间>.png` —— 工具名在最前，`截图` 二字反而在中间。
> 于是前缀 `截图` **只能**匹配裸的 `截图xxx.png`，接不住 `微信截图xxx.png`。
> 必须用完整工具名前缀（`微信截图`、`QQ截图`）才能正确匹配。

新增第三方工具时，加的是它**完整的名字前缀**，不是 `截图` 这个词的变体。
...
- **结束时按文件夹来源做权威替换**（不是只做 `formUnion`）：

  > 扫描每次都重读全部文件夹项（因为不持久化），所以它对该来源的判定是**完整且权威**的。
  > 只 `formUnion` 的话，集合在节点存活期间**只能增不能减** —— 一个原本是截屏、
  > 后被**原地覆盖**成普通照片的文件（同路径、新 mtime、无标记），其 id 仍在列表里
  > 故剪枝保不住它，重扫又找不到它，于是永久留在集合里：「只看截屏」显示出一张相机照片，
  > 且切文件夹也不会清（id 仍然存活）。
  >
  > 讽刺的是 mtime 分支存在的**唯一理由**就是处理原地替换 —— 恰恰在这条路径上集合无法收缩。
  >
  > 因此结束时把合并做成「先摘掉全部文件夹项旧判定，再填回本次命中」，在一次
  > `MainActor.run` 内完成以免中间态闪烁。照片库来源的判定来自加载路径，不参与合并。
  >
  > 该合并规则被抽成纯函数并单测锁定 —— 它是**集合合并规则**的完整性缺口，
  > 既不是接线错误、也不是并发错误，读 diff 与核对触发点都发现不了，
  > 只有把「集合如何变化」整体推演才会暴露。这类缺陷正是纯函数单测的用武之地。
- 照片库来源：不扫文件。改为在加载路径（`PhotoLibraryViewModel.swift:642`）
  把 `loadAllPhotos()` 返回的集合直接赋给 `screenshotPhotoIDs`，且**先于**
  `self.photos = items` —— 因为 `photos` 的 `didSet` 会触发扫描，而扫描入口会剪枝。
- 文件夹来源：`Task.detached` 顺序遍历 `.folder` 项，每扫描 64 **个文件**（而非每 64 次
  **命中**）回主线程合并一次 —— 后者会让截屏稀疏的文件夹长时间停在「已发现 0 张」再突跳；
