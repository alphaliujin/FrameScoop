...
#
# ⚠️ 不要改成生成资源目录 AppIcon.appiconset —— 2026-09-26 实测：
#   Apple 的 App Store 图标提取读的是 bundle 里的 .icns，而 actool 从资源目录
#   导出的 .icns 只有 4 个块（最大 256px），大尺寸全丢，商店页图标会从 1024 掉到 256。
#   本脚本用 iconutil 打包的 .icns 尺寸完整（16→1024 共 11 个块），Apple 取到的就是 1024。
#   详见 project.yml 中 ASSETCATALOG_COMPILER_GLOBAL_ACCENT_COLOR_NAME 上方的说明。
#
